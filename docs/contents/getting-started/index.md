...
The rest of the project file created by `dotnet new` can stay as it is.
...
Create `Components/Layout.razor` to apply the stylesheet and shared page structure:
...
Add `using MySite.Components;` at the top of `Program.cs`, then register the layout before`AddStaticPages()`:
